...
var chat = {
	...
	render : function(template,params){
		...
		switch(template){
			case 'loginTopBar':
				...
				'</span><a href="" class="logoutButton rounded">Вийти</a></span>'];
			break;
	...
	getChats : function(callback){
		$.tzGET('getChats',{lastID: chat.data.lastID},function(r){
			...
			if(!chat.data.lastID){
				chat.data.jspAPI.getContentPane().html('<p class="noChats">В чаті ще нічого не написали</p>');
			}
	...
	getUsers : function(callback){
		$.tzGET('getUsers',function(r){
			...
			if(r.total<1){
				message = 'Відсутні активні користувачі чату';
			}
			else {
				message = 'Авторизовано користувачів: ' + r.total;
			}
